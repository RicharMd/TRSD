...
class DistilConfig(TrainingArguments):
    ...
    alpha: float = field(
        ...
        metadata={
            "help": (
                "Distillation loss and proximal q_tau selection. "
                "0.0 (default): forward KL loss and geometric logit mix for q_tau. "
                "1.0: reverse KL loss and arithmetic probability mix q_tau=(1-tau)q_o+tau p. "
                "Values in between use Jensen-Shannon for the loss; q_tau stays geometric."
            )
        },
    )
    distillation_topk: Optional[int] = field(
        default=None,
        metadata={
            "help": "If set, distill only on the teacher's top-K logits per token (plus optional tail), matching "
            "full-logit KL on a reduced support. `None` keeps full-vocabulary log-softmax (higher memory)."
        },
    )
    distillation_add_tail: bool = field(
        default=True,
        metadata={
            "help": "When `distillation_topk` is set: if True, append one tail bucket so top-K mass + tail sums to 1 "
            "(verl / MS-style). If False, renormalize only over the K logits."
        },
    )
    distillation_divergence: str = field(
        default="kl",
        metadata={
            "help": (
                "Training divergence D_f: 'kl' uses --alpha (0=forward, 1=reverse, else JS-style mix); "
                "'hellinger' uses Squared Hellinger H^2(p, q)=1-sum sqrt(p q) between student and teacher. "
                "For budget_hellinger* closed-loop, set hellinger so D_f matches D_s."
            )
        },
    )
    proximal_selection: str = field(
        default="auto",
        metadata={
            "help": (
                "Proximal teacher path for q_tau: 'auto' follows --alpha (0->forward_kl geometric, "
                "1->reverse_kl arithmetic); 'forward_kl' / 'reverse_kl' force those mixes; "
                "'hellinger' uses the √-mix u=(1-tau)sqrt(q_o)+tau sqrt(p), q_tau=u^2/||u||^2. "
                "budget_hellinger* forces hellinger selection."
            )
        },
    )
    proximal_teacher_tau: float = field(
        default=0.0,
        metadata={
            "help": (
                "Dataset-level proximal teacher weight tau in [0, 1]. "
                "0 uses the original demonstration teacher q_o; 1 uses the current-policy endpoint p. "
                "With proximal_selection=auto and alpha=0 (forward KL), q_tau is the geometric logit mix; "
                "with alpha=1 (reverse KL), q_tau is the arithmetic probability mix "
                "(1-tau)q_o + tau p; with proximal_selection=hellinger, q_tau is the √-mix. "
                "Implemented for full-vocabulary distillation."
            )
        },
    )
    sample_tau_map_path: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Optional JSONL mapping from original dataset index to per-sample proximal teacher tau. "
                "When provided, batch sample_tau overrides dataset-level proximal_teacher_tau."
            )
        },
    )
    sample_metrics_path: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Optional JSONL path for per-sample training signals such as entropy, confidence, "
                "teacher-student KL, and assigned sample tau."
            )
        },
    )
    online_sample_tau_mode: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Optional online tau rule. Supports 'budget_forward_kl' "
                "(tau_i = clip(max(0, 1 - sqrt(C / K_i)), clip_min, clip_max)), "
                "'budget_forward_kl_exp' "
                "(tau_i = clip(tau_max * exp(-sqrt(C / K_i)), clip_min, clip_max) with tau_max=clip_max), and "
                "'budget_forward_kl_relative' "
                "(tau_i = clip(max(0, 1 - sqrt(alpha * K_bar / K_i)), clip_min, clip_max)), "
                "'budget_forward_kl_relative_exp' "
                "(tau_i = clip(tau_max * exp(-sqrt(alpha * K_bar / K_i)), clip_min, clip_max) with tau_max=clip_max), "
                "'budget_forward_kl_relative_exp_range' "
                "(tau_i = tau_min + (tau_max - tau_min) * exp(-sqrt(alpha * K_bar / K_i)) with "
                "tau_min=clip_min, tau_max=clip_max; no post-hoc clamp), "
                "where K_i = mean-token KL(q_o || p_theta) and K_bar is the per-sample batch mean. "
                "'budget_forward_kl_relative_token', 'budget_forward_kl_relative_exp_token', and "
                "'budget_forward_kl_relative_exp_range_token' use "
                "per-token K_{i,t} at loss time with K_bar = per-sample batch mean precomputed on "
                "the full generation batch (same as budget_forward_kl_relative). "
                "'softmax_router_relative' uses "
                "tau_i = clip((K_i/(gamma*K_bar))^alpha / (1 + (K_i/(gamma*K_bar))^alpha), clip_min, clip_max); "
                "'softmax_router_relative_token' applies the same router per token. "
                "Entropy branch (*_entropy*): replace K_i with H_i = mean-token entropy H(p_theta) "
                "and K_bar with H_bar (same formulas). "
                "With --online_sample_tau_k0_path (sample mode only), uses prior anchor: "
                "tau_i = clip(max(0, 1 - sqrt(alpha * (Kbar_0_global/K_i,0)^gamma * (Kbar_t_batch/K_i,t))), "
                "clip_min, clip_max). Kbar_0_global is the mean kl_qo_to_p0 over the full K0 table."
            )
        },
    )
    online_sample_tau_k0_path: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Optional JSONL from compute_tool_train_k0_rollout_kl.py (kl_qo_to_p0 per dataset index). "
                "Requires budget_forward_kl_relative, budget_forward_kl_relative_exp, "
                "or budget_forward_kl_relative_exp_range; "
                "multiplies the budget by (global-mean Kbar_0/K_i,0)^gamma."
            )
        },
    )
    online_sample_tau_k0_gamma: float = field(
        default=1.0,
        metadata={
            "help": (
                "Exponent on the offline K0 prior ratio (Kbar_0_global/K_i,0) in budget_forward_kl_relative "
                "when online_sample_tau_k0_path is set. gamma=1 recovers the original dual-anchor rule."
            )
        },
    )
    online_sample_tau_router_gamma: float = field(
        default=1.0,
        metadata={
            "help": (
                "Kbar scale for softmax_router_relative*: effective ratio is K/(gamma*K_bar) inside the "
                "alpha power. gamma>1 lowers tau (more retention); gamma<1 raises tau."
            )
        },
    )
    online_sample_tau_budget_c: float = field(
        default=0.0,
        metadata={"help": "Budget C used by --online_sample_tau_mode budget_forward_kl / budget_forward_kl_exp."},
    )
    online_sample_tau_budget_alpha: float = field(
        default=0.0,
        metadata={
            "help": (
                "Alpha for relative online tau modes: sqrt-budget scale for budget_forward_kl_relative*; "
                "router temperature for softmax_router_relative*."
            )
        },
    )
    online_sample_tau_clip_min: float = field(
        default=0.0,
        metadata={"help": "Minimum tau for online sample tau rules."},
    )
    online_sample_tau_clip_max: float = field(
        default=1.0,
        metadata={"help": "Maximum tau for online sample tau rules."},
    )
    online_sample_tau_k_bar_ema_decay: float = field(
        default=0.99,
        metadata={
            "help": (
                "EMA decay for batch peer mean K_bar (or H_bar) in relative/router online tau modes. "
                "K_bar_ema = decay * K_bar_ema + (1 - decay) * K_bar_batch. "
                "Set <=0 to disable and use the raw per-batch mean."
            )
        },
    )
    online_sample_tau_chi2_topk: int = field(
        default=64,
        metadata={
            "help": (
                "Top-K support for Pearson chi^2 peer signal in budget_chi2* modes. "
                "Uses top-K indices from the student policy p with one tail bucket (full-softmax masses; "
                "q_o tail is the remaining mass on the same K indices). Set <=0 for full-vocabulary. "
                "Bucket construction is unchanged; chi2/rPE is then applied on those buckets."
            )
        },
    )
    online_sample_tau_chi2_relative_alpha: float = field(
        default=0.1,
        metadata={
            "help": (
                "Relativity mix a for relative-Pearson (rPE) chi2 peer in budget_chi2* modes: "
                "mix = a*q_o + (1-a)*p, peer = E_p[(q_o/mix - 1)^2]. Default 0.1 "
                "(Yamada et al., NIPS 2011). Set 0 for bare E_p[(q_o/p - 1)^2]. "
                "Not the distillation --alpha and not --online_sample_tau_budget_alpha."
            )
        },
    )
    batch_kl_filter_keep_frac: Optional[float] = field(
        default=None,
        metadata={
            "help": (
                "If set in (0, 1], keep the lowest fraction by KL(q_o || p_theta) and mask the rest "
                "from the distillation loss. Scope depends on batch_kl_filter_level: "
                "'sample' ranks rows on the generation batch; 'token' ranks loss tokens within "
                "each sequence. Example: 0.9 keeps the 90% lowest-KL units. "
                "Requires forward KL (alpha=0), tau=0, full-vocabulary distillation."
            )
        },
    )
    batch_kl_filter_level: str = field(
        default="sample",
        metadata={
            "help": (
                "Granularity for batch_kl_filter_keep_frac: 'sample' drops entire sequences; "
                "'token' drops high-KL completion tokens inside each kept sequence."
            )
        },
    )
    batch_kl_filter_drop_weight: float = field(
        default=0.0,
        metadata={
            "help": (
                "Loss weight for filtered-out units when batch_kl_filter_keep_frac is set. "
                "Kept units always weight 1.0; dropped units use this value (0 = hard mask, "
                "e.g. 0.2 or 0.4 = soft down-weight). Must be in [0, 1]."
            )
        },
    )
    effective_tau_metrics_path: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Optional JSONL path for token-level effective tau diagnostics. "
                "For prompt-embedding global direction runs, this projects the shifted teacher "
                "onto the log-prob proximal path from the unshifted teacher to the current policy."
            )
        },
    )
    probe_metrics_path: Optional[str] = field(
        default=None,
        metadata={
            "help": "Optional JSONL path for per-step L_new/L_old probe metrics. Defaults to output_dir/probe_metrics.jsonl."
        },
    )
    probe_every_steps: int = field(
        default=0,
        metadata={"help": "If >0, compute fixed new/old probe losses every N optimizer steps."},
    )
    probe_new_path: str = field(
        default="data/tooluse_data/eval_data",
        metadata={"help": "Dataset path for fixed new-task CE/NLL probe."},
    )
    probe_new_max_samples: int = field(
        default=32,
        metadata={"help": "Maximum number of tool-use examples used for the new-task CE/NLL probe."},
    )
    probe_old_every_steps: int = field(
        default=0,
        metadata={"help": "Deprecated alias for probe_every_steps for old-task IFEval KL probe."},
    )
    probe_old_max_samples: int = field(
        default=32,
        metadata={"help": "Maximum number of IFEval prompts used for the old-task KL probe."},
    )
    probe_old_topk: int = field(
        default=64,
        metadata={"help": "Top-K support used to approximate KL(p_init || p_current) for the old-task probe."},
    )
    ...
    soft_teacher_directions_pt: Optional[str] = field(
        default=None,
        metadata={
            "help": "Path to .pt from prepare_soft_teacher_prompt_directions.py (unit vectors under key 'vectors')."
        },
    )
    soft_teacher_vector: Optional[str] = field(
        default=None,
        metadata={
            "help": "Direction name in the .pt file, e.g. orig_minus_student (Original−question; use w<0 for question−Original)."
        },
    )
    soft_teacher_w: float = field(
        default=0.0,
        metadata={
            "help": "Scalar on the unit direction, broadcast to all non-padding teacher *prompt* token embeddings."
        },
    )
    soft_teacher_row_directions_pt: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Optional .pt with row_directions [num_rows, hidden_size]. "
                "Rows are selected by dataset column soft_teacher_row_id."
            )
        },
    )
    soft_teacher_embed_mix_alpha: Optional[float] = field(
        default=None,
        metadata={
            "help": (
                "If set, prompt embeddings use alpha * original_embedding + (1-alpha) * direction "
                "instead of additive original_embedding + w * direction."
            )
        },
    )
